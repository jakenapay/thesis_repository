...
|---|---|
| **Rate limiting** | 20 requests per 10-minute window per logged-in user. A simple fixed-window counter in CodeIgniter's file cache (key `ai_check_<user_id>`), not a token-bucket throttle — chosen so "requests remaining" is a clean, predictable integer for the UI rather than a continuously-refilling float. Every response (success or failure) includes `requests_remaining`; once it hits `0`, further calls return `429` + `Retry-After` instead of consuming more of the count. The frontend disables the "Check AI Content" button and shows an info icon (`<i class="fas fa-circle-info">` with a Bootstrap tooltip — "Requests left: 0. Try again in a while.") next to it once a response reports `requests_remaining: 0`. This state is reactive (set after a response, including the very first 429) rather than pre-checked on page load, so a fresh page load always starts with the button enabled even if the user is still mid-window from prior activity. |
